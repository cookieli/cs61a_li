...
def valid_symbol(s):
    """Return s is not a well-formed value."""
    if len(s) == 0 or s[0] not in _SYMBOL_STARTS:
        return False
    for i in range(1, len(s)):
        if s[i] not in _SYMBOL_INNERS:
            return False
    return True

def next_candidate_token(line, k):
    """A turple (tok, k), where tok is the next substring of line at or after position k that could be a token (assuming it passes a validity check),and k' is the position in line following that token. Returns (None, len(line)) when there are no more tokens."""
    while k < len(line):
        c = line[k]
        if c == ';':
            ...
            j = k
            while j < len(line) and len(j) not in _TOKEN_END:
                j += 1
