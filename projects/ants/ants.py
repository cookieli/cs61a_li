...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container is True and self.ant is None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.max_range + 1):
            if cur == hive:
                return None
            if i >= self.min_range and len(cur.bees):
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """ A WallAnt that has a high armor value."""
    name = 'Wall'
    food_cost = 4
    implemented = True
    ...
    def action(self, colony):
        ...
        if self.curcount > 1:
            ...
            while place.exit:
                place = place.exit
                if place.ant:
                    if place.ant not in self.has_doubled and hasattr(place.ant, 'damage'):
                        place.ant.damage = place.ant.damage * 2
                        self.has_doubled.append(place.ant)
                    if place.ant.container and place.ant.ant and place.ant.ant not in self.has_doubled and hasattr(place.ant.ant, 'damage'):
                        place.ant.ant.damage = place.ant.ant.damage * 2
                        self.has_doubled.append(place.ant.ant)
